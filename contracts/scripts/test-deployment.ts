...
async function main() {
  console.log("🚀 Testing RemesaPay Functionality");
  ...
  const remesaPay = await upgrades.deployProxy(
    RemesaPayFactory,
    [treasury.address, deployer.address, usdcAddress, ethers.ZeroAddress],
    { initializer: "initialize", kind: "uups" }
  );
  await remesaPay.waitForDeployment();
  const remesaPayAddress = await remesaPay.getAddress();
  console.log("✅ RemesaPay deployed at:", remesaPayAddress);
  ...
  await remesaPay.connect(merchant).registerMerchant(
    "Test Merchant",
    "Quito, Ecuador", 
    merchant.address
  );
  ...
  console.log("✅ Remittance sent and claimed");
